...
def from_awkward(A, *, name=None):
    ...
    The Awkward Array must be in the format generated by `to_awkward`.
    ...
    Vector or Matrix
    """
    if not A.layout.is_RecordType:
        raise ValueError("A must be have a RecordType layout")
    ...
    if len(shape) == 1:
        ...
        assert (A.indices.layout.offsets.data == d["indptr"]).all(), "Mismatched offsets"
        if format[-1] == "r":
            indices = "col"
            labels = "rows"
        else:
